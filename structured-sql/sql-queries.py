...
# FIXED: 'Walk-Out' -> 'Walk Out' (space, not hyphen — confirmed via distinct basement values)
...
conn.close()# 8. Character-style homes built before 1940
# NOTE: 'Character' does not exist in arch_style vocabulary — expect 0 matches by design.
# This is a valid null case: the query cannot be resolved structurally.
queries['q08'] = ("SELECT mls FROM property WHERE year_built < %s AND arch_style ILIKE %s",
                   [1940, '%Character%'])
...
# NOTE: 'Modern' does not exist in arch_style vocabulary — expect 0 matches by design.
queries['q10'] = ("SELECT mls FROM property WHERE type ILIKE %s AND beds >= %s AND list_price < %s AND arch_style ILIKE %s",
                   ['%Detached%', 4, 850000, '%Modern%'])
...
# NOTE: schema has no single 'Split Level' value — uses '2/3/4/5 Level Split' variants.
# Matching on '%Split%' to catch all of them.
queries['q17'] = ("SELECT mls FROM property WHERE arch_style ILIKE %s AND full_baths = %s AND list_price BETWEEN %s AND %s",
                   ['%Split%', 3, 700000, 800000])
...
conn.close()# 8. Character-style homes built before 1940 (Victorian dropped — not a known arch_style)
queries['q08'] = ("SELECT mls FROM property WHERE year_built < %s AND arch_style ILIKE %s",
                   [1940, '%Character%'])
